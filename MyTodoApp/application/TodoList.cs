...
namespace Application
{
      public class TodoList : ITodoList
      {
            ...
            {
                  int id = elements?.Count > 0 ? elements.Count + 1 : 1;
            ...
            public void DoElement(string s)
            {
                  ...
                  {
                        System.Console.WriteLine(string.Format(InfoFormatString, $"{s} is not a valid todo ID. Id must be an integer."));
                        return;
                  }

                  if (id > elements?.Count)
                  {
                        System.Console.WriteLine(string.Format(InfoFormatString, $"Id {id} is not in list."));
                        return ;
                  }
                  else if (id <= 0)
                  {
                        System.Console.WriteLine(string.Format(InfoFormatString, $"Id {id} is not a valid id."));
                        return;
                  }
                  ...
                  {
                        System.Console.WriteLine(string.Format(InfoFormatString, "Todo element is already marked as done."));
                        return;
                  }

                  elementToComplete.MarkAsDone();

                  System.Console.WriteLine("Completed {0}", elementToComplete);
            ...
            public void PrintElement()
            {
                  ...
                  if (allDone)
                  {
                        System.Console.WriteLine(string.Format(InfoFormatString, "All elements is marked as done."));
                  }
